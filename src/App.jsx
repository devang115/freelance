...
function App() {
    ...
    payments.forEach((payment) => {
      if (payment.status === 'paid') {
        ...
        monthlyEarnings[monthYear] =
          (monthlyEarnings[monthYear] || 0) + payment.amount;
      }
    });
  ...
  const deleteProject = (id) => {
    ...
    setPayments(
      payments.filter(
        (payment) =>
          payment.projectName !== projects.find((p) => p.id === id).name
      )
    );
  };
  ...
  const addPayment = (payment) => {
    setPayments([
      ...payments,
      { ...payment, id: Date.now(), paidDate: Date.now() }, 
    ]);
  };
  ...
  const markPaymentAsPaid = (id) => {
    setPayments(
      payments.map((payment) =>
        payment.id === id
          ? { ...payment, status: 'paid', paidDate: Date.now() } 
          : payment
      )
    );
  };
